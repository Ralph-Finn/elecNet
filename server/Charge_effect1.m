function [ Selected,Path,EV,mile,R,arrive_time,leave_time,Wait,SLOW,distance ] = Charge_effect1( Before,CP,Path,Rr,R,mile,EV,K,v,CP_new )
...
global TH_l PATH MILE Mile_dthre ROAD Mile_max 
Dmile=0;
for j=Rr:R-1
...
if Dmile<=Mile_dthre(K)       %在电量允许范围内能够到达目的地则不修改路径
    ...
    YU=sort([Path(R),CP_new(Selected,3)]);
    IU=find(PATH(:,1)==YU(1));
    U=IU(find(PATH(IU,2)==YU(2))); 
    if isempty(U)
        path2=[];
